...
def calcular_orientacion(aceleracion, giroscopio, timestamps):
    roll = [0]
    pitch = [0]
    yaw = [0]
    ...
    for i in range(1, len(timestamps)):
        ...
        # Integración pura del giroscopio (sin filtro complementario)
...
def process(samples):
    ...
    roll, pitch, yaw = calcular_orientacion(aceleracion, giroscopio, tiempo_norm)

    # Ajustar polinomios
    t_interp = np.linspace(tiempo_norm[0], tiempo_norm[-1], 100)

    pol_alt = ajustar_polinomio(tiempo_norm, altitudes, grado=3)
    alt_ajustada = pol_alt(tiempo_norm)
    alt_interp = pol_alt(t_interp)

    pol_roll = ajustar_polinomio(tiempo_norm, roll, grado=3)
    roll_interp = pol_roll(t_interp)

    pol_pitch = ajustar_polinomio(tiempo_norm, pitch, grado=3)
    pitch_interp = pol_pitch(t_interp)

    pol_yaw = ajustar_polinomio(tiempo_norm, yaw, grado=3)
    yaw_interp = pol_yaw(t_interp)

    # Derivar en base a altitud ajustada
